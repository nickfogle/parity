...
impl Database {
	...
	pub fn write_buffered(&self, mut tr: DBTransaction) -> Result<(), String> {
		let mut overlay = self.overlay.write();
		let ops = mem::replace(&mut *tr.ops.get_mut(), Vec::new());
		for op in ops {
			match op {
	...
	/// Commit transaction to database.
	pub fn write(&self, mut tr: DBTransaction) -> Result<(), String> {
		let batch = WriteBatch::new();
		let ops = mem::replace(&mut *tr.ops.get_mut(), Vec::new());
		for op in ops {
			match op {
